# test_combined.py -- OLED + Joystick + Resistor-ladder buttons combined
# ======================================================================
# Shows joystick + button-ladder state on the OLED using fast direct
# framebuffer writes.
#
# Encoder is INTENTIONALLY excluded from this test. Rationale:
# test_ec11.py confirms the EC11 decodes cleanly on its own with simple
# digitalio polling at ~1 ms. Combining it with the joystick here pushes
# OLED redraws into every loop iteration (joystick ADC values change
# constantly), which stretches each iteration to ~10-15 ms -- longer
# than one encoder A/B phase, so quadrature direction becomes unreliable.
# The real DMX controller UI updates on slower events (menu changes,
# XBee replies, button presses) rather than continuous analog values, so
# the encoder will get the timing it needs in the main application. This
# test only validates joystick + buttons + OLED coexistence.
#
...
# -- Main loop ---------------------------------------------------------
print("Combined test (joystick + buttons + OLED) -- Ctrl+C to stop")
...
try:
    while True:
        ...
        # Clear and redraw dynamic pages
    ...
    fast_text("Btn presses: " + str(press_count), 0, 3)
    oled.show()
    print("\nDone. " + str(press_count) + " btn presses.")
