...
# D6 (TX) → XBee DIN, D7 (RX) ← XBee DOUT
...
if not resp or b"OK" not in resp:
    ...
    # Stop execution — no point testing comms if module isn't responding
    raise SystemExit

# Read current network settings for display
# Each AT command without a value parameter returns the current setting
uart.write(b"ATMY\r")   # Read MY (this module's address)
time.sleep(0.3)
my = uart.read(64)
...
# Anything typed in XCTU Console on the PC should arrive here via
# the wireless link (PC's DL points to our MY address).
print("\n[3] Listening for data from PC (type in XCTU Console)...")
print("    Press Ctrl+C to stop.\n")

while True:
    data = uart.read(64)  # Read up to 64 bytes, returns None if timeout
    if data:
        try:
            text = data.decode("utf-8").strip()
            print("  RX: " + text)
        except Exception:
            # If data isn't valid UTF-8, show raw bytes
